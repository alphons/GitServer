...
/// <summary>Puts the configurable git path prefix (e.g. "/git") in front of the git smart-HTTP routes only,
...
public class GitRoutePrefixConvention(string prefix) : IControllerModelConvention
{
	...
	{
		if (prefix.Length == 0 || controller.ControllerType != typeof(GitController)) return;
