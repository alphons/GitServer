...
public class AccessTokensEndToEndTests : IClassFixture<GitServerFactory>
{
	...
	private async Task<string> CreateTokenAsync(AppUser user, string name = "laptop", string days = "90")
	{
		...
		var response = await session.PostFormAsync("/dashboard/User/AccessTokens", "/dashboard/User/AccessTokens?handler=CreateToken", ("TokenName", name), ("TokenValidDays", days));
	...
	public async Task TheTokenIsShownOnce_TheListOnlyHasItsName()
	{
		...
		var html = await session.GetHtmlAsync("/dashboard/User/AccessTokens");
	...
	public async Task ARevokedToken_StopsWorking_AndOthersKeepWorking()
	{
		...
		await session.PostFormAsync("/dashboard/User/AccessTokens", $"/dashboard/User/AccessTokens?handler=RevokeToken&id={id}");
	...
	public async Task YouCannotRevokeSomeoneElsesToken()
	{
		...
		await (await new WebSession(_f).LoginAsync(other.UserName!)).PostFormAsync("/dashboard/User/AccessTokens", $"/dashboard/User/AccessTokens?handler=RevokeToken&id={id}");
	...
	public async Task ATokenNeedsAName()
	{
		...
		await session.PostFormAsync("/dashboard/User/AccessTokens", "/dashboard/User/AccessTokens?handler=CreateToken", ("TokenName", "   "), ("TokenValidDays", "90"));
